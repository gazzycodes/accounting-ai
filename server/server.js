...
app.get('/api/dashboard', async (req, res) => {
  try {
    // Use current month date range for consistency with reports
    const now = new Date()
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)
    const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0)

    const financialData = await FinancialDataService.getFinancialData(startOfMonth, endOfMonth)

    // Generate sparkline data - mix real transactions with mock data for demo
    ...
    if (recentTransactions.length > 0) {
      ...
      sparklineData = recentTransactions.map(t => ({
...
// --- Reports (Balance Sheet using centralized data) ---
app.get('/api/reports/balance-sheet', async (req, res) => {
  try {
    const asOf = new Date().toISOString().slice(0, 10)
    
    // Use centralized financial data for consistency
...
app.get('/api/reports/pnl', async (req, res) => {
  try {
    ...
    const sum = (xs) => xs.reduce((acc, x) => acc + x.amount, 0)
    const totals = {
      revenue: Number(sum(financialData.revenueBreakdown).toFixed(2)),
      expenses: Number(sum(financialData.expenseBreakdown).toFixed(2)),
    }
    const grossProfit = Number((totals.revenue - (financialData.expenseBreakdown.find(e => e.name.toLowerCase().includes('cost of goods') || e.name.toLowerCase().includes('cogs'))?.amount || 0)).toFixed(2))
    const pnlNetProfit = Number((totals.revenue - totals.expenses).toFixed(2))

    res.json({ 
      period, 
      revenue: financialData.revenueBreakdown, 
      expenses: financialData.expenseBreakdown, 
      totals: { ...totals, grossProfit, netProfit: pnlNetProfit } 
    })
  } catch (error) {
...
async function processAIChat(message, context = {}) {
  try {
    // Get real financial data for AI context
    const now = new Date()
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)
    const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0)
