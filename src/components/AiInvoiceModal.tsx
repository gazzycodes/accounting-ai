...
const AiInvoiceModal = ({ isOpen, onClose }: AiInvoiceModalProps) => {
  ...
  useEffect(() => {
    const loadClients = async () => {
      try {
        ...
        const clientsData = await FinancialDataService.getCustomers()
  ...
  const handleAddNewClient = async () => {
    try {
      ...
      const result = await FinancialDataService.addCustomer({
        ...
        city: newClientData.city?.trim() || undefined,
        state: newClientData.state?.trim() || undefined,
        zipCode: newClientData.zipCode?.trim() || undefined,
        notes: newClientData.notes?.trim() || undefined
      })

      // Add to local state
      const newClient = result.customer
      setClients(prev => [...prev, newClient])
      
      // Select the new client
      setSelectedClient(newClient)
      
      // Reset form and close add client modal
      setNewClientData({
        name: '',
        company: '',
        email: '',
      ...
      console.log('✅ New client added and selected:', newClient.name)
      
    } catch (error) {
      console.error('❌ Failed to add client:', error)
      setSaveError((error as Error).message || 'Failed to add client')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const handleUpdateClient = async () => {
    try {
      ...
      const result = await FinancialDataService.updateCustomer(editingClient.id, {
        name: editClientData.name?.trim() || '',
        company: editClientData.company?.trim() || undefined,
        email: editClientData.email?.trim() || '',
        phone: editClientData.phone?.trim() || undefined,
        address: editClientData.address?.trim() || undefined,
        city: editClientData.city?.trim() || undefined,
        state: editClientData.state?.trim() || undefined,
        zipCode: editClientData.zipCode?.trim() || undefined,
        notes: editClientData.notes?.trim() || undefined
      })

      // Update local state
      const updatedClient = result.customer
      ...
      // Reset form and close edit client modal
      setEditClientData({
        name: '',
        company: '',
        email: '',
      ...
      setSaveError((error as Error).message || 'Failed to update client')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const handleSendInvoice = async () => {
    ...
    try {
      ...
      if (lineItems.some(item => item.amount <= 0)) {
        throw new Error('All line items must have positive amounts')
      }

      // Calculate totals (reusing existing calculation logic)
      const subtotal = lineItems.reduce((sum, item) => sum + item.amount, 0)
      const discountAmount = invoiceData.discount.enabled 
        ? invoiceData.discount.type === 'percentage'
          ? subtotal * (invoiceData.discount.value / 100)
      ...
      const invoicePayload = {
        customerName: selectedClient?.company || selectedClient?.name || 'Unknown Customer',
        amount: total,
        subtotal: subtotal,
        date: invoiceData.date,
        description: invoiceData.notes || `Invoice for ${selectedClient?.company}`,
        invoiceNumber: invoiceData.invoiceNumber,
        dueDate: invoiceData.dueDate,
        category: 'PROFESSIONAL_SERVICES',
        paymentStatus: 'invoice' as 'invoice',
        // Tax information
        taxSettings: invoiceData.taxSettings.enabled ? {
          enabled: true,
          name: invoiceData.taxSettings.name || 'Sales Tax',
          type: invoiceData.taxSettings.type,
          rate: invoiceData.taxSettings.rate,
          amount: taxAmount,
          taxExempt: invoiceData.taxSettings.taxExempt
        } : { enabled: false },
        // Discount information  
        discount: invoiceData.discount.enabled ? {
          enabled: true,
          description: invoiceData.discount.description || 'Discount',
        ...
        lineItems: lineItems.map(item => ({
          ...
          rate: item.rate,
          category: 'PROFESSIONAL_SERVICES' // Default category for now
        }))
      }
      ...
      if (result.success) {
        // Success handling
        const successMessage = `✅ Invoice ${invoiceData.invoiceNumber} created successfully!\n\n` +
          `Customer: ${selectedClient?.company}\n` +
          `Amount: $${total.toFixed(2)}\n` +
          `Transaction ID: ${result.transaction?.transactionId || 'Generated'}`
    ...
    onClose()
      ...
      setSaveError((error as Error).message || 'Failed to create invoice')
    } finally {
      setIsSaving(false)
    }
  }
        ...
        >
                ...
                >
                  ...
                  <div className="mb-6 space-y-4">
                    ...
                    <motion.button
                      ...
                      className="w-full py-3 bg-gradient-to-r from-emerald-600/20 to-teal-600/20 hover:from-emerald-600/30 hover:to-teal-600/30 border border-emerald-500/30 hover:border-emerald-400/50 text-emerald-300 hover:text-emerald-200 rounded-xl transition-all duration-200 flex items-center justify-center space-x-2"
                  ...
                  <div className="min-h-[300px]">
                    {isLoadingClients ? (
                      ...
                      <div className="text-center py-12">
                        ...
                        <motion.button
                          onClick={() => setShowAddClient(true)}
                          className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl transition-colors flex items-center space-x-2 mx-auto"
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-emerald-300 text-sm">{client.company || 'Individual'}</p>
                            </div>
                          </div>
                          <button
                            onClick={(e) => {
                            ...
                            title="Edit Client"
                          >
                            <Edit className="h-4 w-4 text-slate-400 hover:text-emerald-400" />
                          </button>
                        </div>
                  ...
                  <div className="bg-white rounded-xl p-8 text-black max-h-[600px] overflow-y-auto">
                      ...
                      <div>
                        ...
                        <div className="text-gray-600">
                          ...
                          <p className="text-sm font-medium">Status: <span className={`${
                            ...
                            'text-gray-600'
                          }`}>{invoiceData.status?.toUpperCase() || 'DRAFT'}</span></p>
                        </div>
                      </div>
                    ...
                    <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        ...
                        <p>Invoice Status: {invoiceData.status?.toUpperCase() || 'DRAFT'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between mt-8">
                    ...
                    <div className="flex space-x-4">
                      ...
                      <motion.button
                        ...
                        disabled={isSaving}
                        className={`px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl transition-colors flex items-center space-x-2 ${isSaving ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          {showAddClient && (
            ...
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="bg-gradient-to-br from-slate-900/95 via-slate-800/95 to-slate-900/95 backdrop-blur-xl border border-emerald-500/20 rounded-2xl shadow-2xl shadow-emerald-500/10 p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-emerald-300 text-sm">Create a new client profile</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowAddClient(false)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 focus:border-emerald-400/50 transition-all"
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      className="w-1/2 px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 focus:border-emerald-400/50 transition-all"
                      placeholder="CA"
                    />
                    <input
                      type="text"
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 focus:border-emerald-400/50 transition-all resize-none"
              ...
              <div className="flex justify-between items-center mt-8 pt-6 border-t border-slate-700/50">
                <div className="flex items-center space-x-2 text-slate-400 text-sm">
                  ...
                  <span>All fields are securely stored</span>
                </div>
                <div className="flex space-x-4">
                  <motion.button
                    onClick={() => setShowAddClient(false)}
                    className="px-6 py-3 bg-slate-700/30 hover:bg-slate-600/50 border border-slate-600/50 text-slate-300 hover:text-white rounded-xl transition-all duration-200"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    Cancel
                  ...
                  <motion.button
                    ...
                    disabled={isSaving || !newClientData.name?.trim() || !newClientData.email?.trim()}
                    className={`px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-xl transition-all duration-200 flex items-center space-x-2 shadow-lg shadow-emerald-500/25 ${isSaving || !newClientData.name?.trim() || !newClientData.email?.trim() ? 'opacity-50 cursor-not-allowed' : ''}`}
                    whileHover={!isSaving && newClientData.name?.trim() && newClientData.email?.trim() ? { scale: 1.02 } : {}}
                    whileTap={!isSaving && newClientData.name?.trim() && newClientData.email?.trim() ? { scale: 0.98 } : {}}
                  >
                    {isSaving ? (
                      <>
          ...
          {showEditClient && (
            ...
            >
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-blue-300 text-sm">Update client information</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowEditClient(false)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all"
                    placeholder="John Smith"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      className="w-1/2 px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all"
                      placeholder="CA"
                    />
                    <input
                      type="text"
                      value={editClientData.zipCode}
                      onChange={(e) => setEditClientData(prev => ({ ...prev, zipCode: e.target.value }))}
                      className="w-1/2 px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all"
                      placeholder="94105"
                    />
                  </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all resize-none"
                    placeholder="Additional notes about this client..."
                  />
                </div>
              </div>
              ...
              <div className="flex justify-between items-center mt-8 pt-6 border-t border-slate-700/50">
                <div className="flex items-center space-x-2 text-slate-400 text-sm">
                  <Sparkles className="h-4 w-4" />
                  <span>Changes will be saved securely</span>
                </div>
                <div className="flex space-x-4">
                  <motion.button
                    onClick={() => setShowEditClient(false)}
                  ...
                  <motion.button
                    ...
                    disabled={isSaving || !editClientData.name?.trim() || !editClientData.email?.trim()}
                    className={`px-6 py-3 bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 text-white rounded-xl transition-all duration-200 flex items-center space-x-2 shadow-lg shadow-blue-500/25 ${isSaving || !editClientData.name?.trim() || !editClientData.email?.trim() ? 'opacity-50 cursor-not-allowed' : ''}`}
                    whileHover={!isSaving && editClientData.name?.trim() && editClientData.email?.trim() ? { scale: 1.02 } : {}}
                    whileTap={!isSaving && editClientData.name?.trim() && editClientData.email?.trim() ? { scale: 0.98 } : {}}
                  >
                    {isSaving ? (
                      <>
                        <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
