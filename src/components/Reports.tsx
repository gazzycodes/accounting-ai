...
const getBalanceDisplay = (account: ChartOfAccountsItem) => {
  ...
  return {
    ...
    displayText: `$${absBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}`
  }
}
...
export default function Reports() {
  ...
  // Account Ledger transaction state (moved to top level to fix hook rules)
  ...
  // Database-driven reports with automatic refresh
  ...
  useEffect(() => {
    const fetchInsights = async () => {
      try {
        ...
        switch (tab) {
          case 'pnl':
            insights = await FinancialDataService.getPnlInsights()
            break
          case 'balance':
            insights = await FinancialDataService.getBalanceSheetInsights()
            break
          case 'trial':
            insights = await FinancialDataService.getTrialBalanceInsights()
            break
          case 'coa':
            insights = await FinancialDataService.getChartOfAccountsInsights()
            break
          default:
            insights = await FinancialDataService.getAIInsights()
            break
        }
        setAiInsights(insights || []) // Ensure it's always an array
      } catch (error) {
        console.error('Failed to fetch AI insights:', error)
        setAiInsights([]) // Fallback to empty array on error
      }
    }
  ...
  // Fetch account transactions when selectedAccount changes
  useEffect(() => {
    const fetchAccountTransactions = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:4000/api/accounts/${selectedAccount.code}/transactions`)
  ...
  // Removed getMockTransactions - now using real API data from /api/accounts/:code/transactions

  // Clean AI account analysis - no mock data
  const getAccountAnalysis = (account: ChartOfAccountsItem): AccountAnalysis => {
    // Phase 1: Focus on account structure, detailed analytics in Phase 2
    // Return basic analysis for clean database-first system
    ...
    // Simple analysis based on account balance (Phase 1)
    if (account.balance === 0) {
      aiInsight = `${account.name} is ready for transactions. Clean database-first setup complete.`
      suggestions = [
        'Account is properly configured',
        'Ready for Phase 2 transaction posting',
        'Professional Chart of Accounts established'
      ]
    } else {
      trend = account.balance > 0 ? 'increasing' : 'decreasing'
      percentChange = Math.abs(account.balance / 100) // Simple indicator
      aiInsight = `${account.name} has current balance of $${account.balance.toLocaleString()}.`
      suggestions = [
        'Account shows activity',
        'Balance will be managed in Phase 2',
  ...
  const handleCreateAccount = () => {
    ...
    // TODO: Phase 2 - Implement account creation via backend API
    console.warn('Account creation will be available in Phase 2 via backend API')
    
    // For now, show a message to the user
    alert('Account creation will be available in Phase 2. Currently, accounts are managed through the database seeder.')
      ...
      return (
        <div className="relative">
          ...
          >
            ...
            {/* AI Insights Panel */}
            <AnimatePresence>
              {showAiInsights && aiInsights.length > 0 && aiInsights[currentInsightIndex] && (
                <motion.div
                  className="mb-6"
                  initial={{ opacity: 0, y: -20, scale: 0.95 }}
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              aiInsights[currentInsightIndex].urgency === 'high' ? 'bg-red-500/20 text-red-300' :
                              aiInsights[currentInsightIndex].urgency === 'medium' ? 'bg-yellow-500/20 text-yellow-300' :
                              'bg-green-500/20 text-green-300'
                            }`}>
                              {aiInsights[currentInsightIndex].urgency.toUpperCase()}
                            </span>
                          </div>
                          <motion.p 
                            key={aiInsights[currentInsightIndex].id}
                            className="text-gray-200 text-sm leading-relaxed font-medium"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            transition={{ duration: 0.8 }}
                          >
                            <TypewriterText text={aiInsights[currentInsightIndex].message} />
                          </motion.p>
                          <div className="mt-2 text-xs text-gray-400">
                            {aiInsights[currentInsightIndex].category} • AI Analysis
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="glass p-6 rounded-lg mb-6">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className={`font-bold text-lg ${
                          getBalanceDisplay(selectedAccount).isPositive ? 'text-green-400' : 'text-red-400'
                        }`}>
                          {getBalanceDisplay(selectedAccount).displayText}
                        </span>
                            </div>
                            <div className="flex justify-between items-center">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-white text-sm font-medium">
                                {transactionsLoading ? 'Loading...' : `${accountTransactions.length} entries`}
                              </span>
                            </div>
                            <div className="flex justify-between items-center">
                          ...
                          if (transactionsLoading) {
                            return (
                              ...
                              >
                                ...
                                <h3 className="text-gray-300 font-medium mb-2">Loading Transactions...</h3>
                              </motion.div>
                            )
                          }
                    ...
                    <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 mb-6">
                      ...
                      <div className="text-gray-300 text-sm">
                        Current Balance: <span className={`font-bold ${
                          getBalanceDisplay(accountToDelete).isPositive ? 'text-green-400' : 'text-red-400'
                        }`}>
                          {getBalanceDisplay(accountToDelete).displayText}
                        </span>
                      </div>
                    </div>
